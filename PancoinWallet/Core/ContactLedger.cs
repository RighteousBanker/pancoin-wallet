...
namespace Panode.Core
{
    public class ContactLedger
    {
        ...
        LinearDictionaryCoder coder = new LinearDictionaryCoder(path, hostnameLength, 4);
        ...
        public ContactLedger(HttpProvider httpProvider)
        {
            ...
            {
                AddContact(Program.Settings.defaultNode);
            }
        }
        ...
        {
            return new Tuple<string, DateTime>(Encoding.UTF8.GetString(entry.Take(entry.Length - 4).ToArray()), DateTimeOffset.FromUnixTimeSeconds(ByteManipulator.GetUInt32(entry.Skip(128).ToArray())).DateTime);
        }
