...
namespace Panode.Core
{
    ...
    {
        LinearDictionaryCoder balanceCoder = new LinearDictionaryCoder("balance", 33, 8);
        LinearDictionaryCoder nonceCoder = new LinearDictionaryCoder("nonces", 33, 4);
        ...
        public Dictionary<byte[], Tuple<long, LargeInteger>> CreateDiff(List<Transaction> transactions, Direction direction, Dictionary<byte[], Tuple<long, LargeInteger>> initialDiff = null)
        {
            ...
            if (ret != null)
            {
                foreach (var kvp in balances)
                {
                    ...
                    {
                        nonce = -1; //-1 means that nonce does not exist -> nothing will be stored on disk, it is account which only recieved payments/mining reward
                    }
